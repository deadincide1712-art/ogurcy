...
const DICT_EN = {
  ...
  'Кооператив: вы против волн огурцов-захватчиков. Каждая волна сильнее, каждая пятая — с боссом. Погибшие возвращаются в передышке между волнами.':
    'Co-op: you against waves of invader cucumbers. Every wave is tougher, every fifth has a boss. Fallen players come back during the break between waves.',
  'Выдать и надеть': 'Give and equip', 'Выдать все скины': 'Give all skins', 'Какой скин выдать': 'Which skin to give',
  'Выданы все скины — выбирай в разделах «Огурец», «Оружие» и «Ножи».': 'All skins granted — pick them on the "Cucumber", "Guns" and "Knives" tabs.', 'Кошелёк': 'Wallet', 'Огуречный кейс': 'Cucumber case', 'Открыть · 100 🪙': 'Open · 100 🪙',
...
const PATTERNS = [
  ...
  [/^(\d+) мс$/, (m) => `${m[1]} ms`],
  [/^Волна (\d+)$/, (m) => `Wave ${m[1]}`],
  [/^Волна (\d+) · БОСС: Огурец-мутант!$/, (m) => `Wave ${m[1]} · BOSS: Mutant Cucumber!`],
  [/^Волна (\d+) отбита! Передышка (\d+) с$/, (m) => `Wave ${m[1]} held! ${m[2]} s break`],
  [/^Нашествие! Первая волна через (\d+) с$/, (m) => `Invasion! First wave in ${m[1]} s`],
  [/^Отбито волн: (\d+) · рекорд: (\d+)$/, (m) => `Waves held: ${m[1]} · best: ${m[2]}`],
  [/^· захватчиков: (\d+)$/, (m) => `· invaders: ${m[1]}`],
  [/^передышка · волна (\d+) через$/, (m) => `break · wave ${m[1]} in`],
  [/^рекорд: (\d+) · каждая пятая волна — с боссом$/, (m) => `best: ${m[1]} · every fifth wave has a boss`],
  [/^Скин выдан и надет: (.+)$/, (m) => `Skin granted and equipped: ${T(m[1])}`],
